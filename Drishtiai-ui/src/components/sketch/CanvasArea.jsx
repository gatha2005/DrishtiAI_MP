...
export default function CanvasArea({
  ...
  useEffect(() => {
    console.log("Canvas received:", autoComponents);
    ...
    const addComponent = (src, zoneId) => {

        const zone = SNAP_ZONES.find(z => z.id === zoneId);
        ...
        generated.push({
            ...
            x: zone.x - zone.snapSize / 2,

            y: zone.y - zone.snapSize / 2,

            size: zone.snapSize,

            zIndex: generated.length + 1,
    ...
    addComponent(autoComponents.face, "face");
    addComponent(autoComponents.hair, "hair");
    addComponent(autoComponents.eyes, "eyes");
    addComponent(autoComponents.nose, "nose");
    addComponent(autoComponents.lips, "lips");
  ...
  // Notify parent of canvas data changes
  useEffect(() => {
    if (onCanvasDataChange) {
      onCanvasDataChange({ items, canvasWidth: CW, canvasHeight: CH, canvasRef: renderCanvasRef });
    }
  }, [items, onCanvasDataChange]);

  // Render canvas with proper blend modes
  useEffect(() => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (resizingId) {
      ...
      setItems(prev => prev.map(item => item.id === resizingId ? { ...item, size: Math.max(40, startSize + diff) } : item));
    }
  };
  ...
  const sendBack = () => {
    ...
    setItems(prev => {
      ...
      return prev.map(item => item.id === selectedId ? { ...item, zIndex: Math.max(0, minZ - 1) } : item);
    });
  };
  ...
  return (
    <div style={styles.wrapper}>
      ...
      {selectedItem && (
        <div style={styles.itemControls}>
          <div style={styles.controlGroup}>
            ...
            <input type="range" min="0.1" max="1" step="0.05" value={selectedItem.opacity} onChange={e => updateItem(selectedItem.id, { opacity: parseFloat(e.target.value) })} style={styles.slider} />
      ...
      <div className="canvas" style={styles.canvas} onDrop={handleDrop} onDragOver={handleDragOver} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
        {showZones && SNAP_ZONES.map(zone => {
          const isHovered = hoveredZone === zone.id;
          return (
            <div key={zone.id} style={{ position: "absolute", left: zone.x - zone.w / 2, top: zone.y - zone.h / 2, width: zone.w, height: zone.h,
              background: isHovered ? zone.color.replace(/[\d.]+\)$/, "0.18)") : zone.color, border: `1px dashed ${zone.border}`,
              borderRadius: zone.id === "face" ? "50% / 45%" : "6px", pointerEvents: "none", transition: "background 0.2s",
              display: "flex", alignItems: "flex-start", justifyContent: "flex-end", padding: "4px 6px", boxSizing: "border-box" }}>
        ...
        {[...items].sort((a, b) => a.zIndex - b.zIndex).map(item => (
          <div key={item.id} onMouseDown={e => handleMouseDownMove(e, item.id)} style={{
            ...
            border: selectedId === item.id ? "1.5px solid #f5a623" : "1.5px solid transparent", boxSizing: "border-box",
            cursor: "move", zIndex: item.zIndex, opacity: item.opacity, mixBlendMode: item.blendMode, borderRadius: "2px",
      ...
      <div style={styles.hintBar}>
        {selectedItem ? <>Selected layer <span style={{ color: "#f5a623" }}>Z:{selectedItem.zIndex}</span> · Use <span style={{ color: "#f5a623" }}>BLEND</span> + <span style={{ color: "#f5a623" }}>OPACITY</span> to blend features naturally</>
...
const styles = {
  ...
  toast: { position: "absolute", alignSelf: "center", background: "rgba(0,229,160,0.15)", border: "1px solid rgba(0,229,160,0.4)", borderRadius: "4px", padding: "5px 14px", fontFamily: "var(--font-mono)", fontSize: "10px", color: "#00e5a0", letterSpacing: "1px", zIndex: 9999, animation: "fadeUp 0.3s ease-out", marginBottom: "4px" },
